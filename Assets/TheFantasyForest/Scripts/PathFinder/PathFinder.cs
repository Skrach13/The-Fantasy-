...
public class PathFinder : MonoBehaviour
{
    ...
    */
    ...
    /// <param name="finish">конечная вершина графа</param>
    /// <returns></returns>
    public static List<Vector2> Path(CellBase[,] massiveField, Vector2 start, Vector2 finish)
    {
        List<Vector2> path = FindPath(massiveField, start, finish);
        return path;
    }
    /// <summary>
    ...
    public static void PaintPath(List<Vector2> path, CellBase[,] massiveField, Color color)
    {
        foreach (Vector2 coordinat in path)
        {            
            massiveField[(int)coordinat.x, (int)coordinat.y].PaintCell(color);
        }
    }
    ...
    /// <returns></returns>
    private static int[,] massiveGraff(CellBase[,] massiveFields)
    {
    ...
    /// <returns></returns>
    private static List<Vector2> FindPath(CellBase[,] field, Vector2 start, Vector2 goal)
    {
         ...
         Vector2 goal, CellBase[,] field)
    {
        ...
        foreach (var point in neighbourPoints)
        {
            ...
            if (field[(int)point.x, (int)point.y] != null && (field[(int)point.x, (int)point.y].CloseCell == true))
                continue;
