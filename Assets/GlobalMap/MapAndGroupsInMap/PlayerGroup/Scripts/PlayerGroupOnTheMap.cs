...
public class PlayerGroupOnTheMap : MonoBehaviour
{
    ...
    private List<GlobalMapCell> _path;
    public List<GlobalMapCell> Path { get => _path; set => _path = value; }
    ...
    private void StartMove(List<GlobalMapCell> cells) 
    {
        if (_isMove == false)
        {
            _isMove = true;
    ...
    private IEnumerator MovedInPath(List<GlobalMapCell> cells)
    {
        int i = 0;
       // while (_path.Count != 0)
        while (i < cells.Count)
